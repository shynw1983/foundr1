...
export function InventoryComparisonPreview({comparison,language}:{comparison:InventoryComparison;language:string}) {
 ...
 const rows=comparison.rows.filter(r=>filter==='staged'?staged(r):filter==='unknown'?unknown(r):filter==='excluded'?excluded(r):filter==='differences'?r.changes.length>0:r.changes.includes(filter));
 ...
 return <div>
  ...
  {comparison.rows.some(staged)&&<p className="inventory-state-tag is-neutral">{label('未公開の項目は今回の販売状態同期では変更しません。販売開始は別途メニュー公開が必要です。','未上架项目不会被本次缺货同步修改，也不会自动上架；其他已确认项目可正常同步。','未上架項目不會被本次缺貨同步修改，也不會自動上架；其他已確認項目可正常同步。')}</p>}
