...
export function InventoryComparisonPreview({comparison,language}:{comparison:InventoryComparison;language:string}) {
 ...
 return <div>
  ...
  {comparison.rows.some(staged)&&<p className="inventory-state-tag is-neutral">{label('Uber で販売中の新品は、確認後に公開します。Uber で売切の新品と公開先が未確認の項目は非公開のままです。','Uber 可售且上架位置已核验的新品，将在确认后上架。Uber 缺货或上架位置未确认的新品继续隐藏，不影响其他已确认项目。','Uber 可售且上架位置已核驗的新品，將在確認後上架。Uber 缺貨或上架位置未確認的新品繼續隱藏，不影響其他已確認項目。')}</p>}
