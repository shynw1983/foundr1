...
export function WholeStoreAvailabilitySync({storeId,language,disabled,onApplied}:{storeId:string;language:string;disabled:boolean;onApplied:()=>void}) {
  ...
  return <details className="panel" data-i18n-ignore>
    ...
    {report?.status==='awaiting_confirmation'&&<div>
      <p>{label('確認待ち · まだ変更していません','等待确认 · 尚未修改','等待確認 · 尚未修改')}</p>
      {[true,false].map(available=>{
        const rows=report.details.preview?.filter(t=>t.isAvailable===available)??[];
        const changed=rows.filter(t=>t.wasAvailable!==null&&t.wasAvailable!==available).length;
        return <details key={String(available)}><summary>{available?label('Uber 販売中','Uber 可售','Uber 可售'):label('Uber 売切','Uber 缺货','Uber 缺貨')}: {rows.length} · {label('Store から変更','相较 Store 将变更','相較 Store 將變更')} {changed}</summary>
          <ul>{rows.map((t,i)=><li key={i}>{t.label}{t.wasAvailable!==t.isAvailable?' → '+(available?label('販売再開','恢复销售','恢復銷售'):label('売切','缺货','缺貨')):''}</li>)}</ul></details>;
      })}
      <button className="primary-button" type="button" disabled={busy||disabled} onClick={()=>void confirm()}>{label('内容を確認して実行','确认变更并执行','確認變更並執行')}</button>
    </div>}
