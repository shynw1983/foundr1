...
Only `inside`, `outside` or `unknown`, rule identity and observation time leave the phone. GPS samples and travel history are not stored on the server. Push registration binds to an active employee session. The background API receives an opaque device capability; its hash is stored in the database, and it can only refresh that device's token and presence. It cannot read orders, change rules or acknowledge alerts. Logout/revoked sessions prevent further dispatch. Backup is disabled for the STORE flavor so these credentials are not restored to another phone.
