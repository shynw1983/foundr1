...
販売可否に加えて、Foundr1 OS のメニュー配信センターから現在メニューの基準取込と、確認済み差分の
配信タスクを受け取ります。名称・価格・追加・配置など、対象プラットフォームでまだ実機検証されて
いない変更種別は成功扱いにせず `menu_action_unsupported` で停止します。
...
- メニュー基準取込で一致しない対象が一件でもある場合は、構造変更の一括配信を開始しません。
- ログイン切れやページ構造変更時は停止し、推測した座標をクリックしません。

### 出前館の自動再ログイン

出前館がログアウトしていることを Bridge が確認した場合だけ、Mac のログインキーチェーンから
「コード」「ログイン ID」「パスワード」を読み取り、自動で一度だけ再ログインします。
認証情報は `config.local.json`、ソースコード、ログには保存しません。設定は Bridge を実行する
Mac ユーザー自身が Terminal で行います（チャットには貼り付けないでください）。

```bash
npm run credentials:demae
npm run check:demae
```

認証失敗後はアカウントロックを避けるため 30 分間再試行しません。パスワード不一致、アカウント
ロック、仮パスワード期限切れ、または将来 CAPTCHA／二段階認証が表示された場合は自動操作を
停止し、Store App に要対応として表示します。通常の一日程度のセッション切れは自動復旧します。
...
競合店メニュー監視もこの Uber Eats 用 Chrome プロファイルを使用します。最初に公開側の
Uber Eats で監視専用の配達先を設定してください。Bridge は店舗が営業中で、配達先が設定済み、
かつ商品カードを完全に読み取れた場合だけ「割引終了」を確定します。営業時間外、配達先未設定、
HTTP 403、またはメニューの一部しか読めない場合は前回の割引状態を保持します。

## Siri / macOS Shortcuts

音声操作は Bridge の端末トークンをそのまま利用し、ショートカット側には認証情報を保存しません。
実行前にショートカットで商品名を聞き、中国語で商品名と操作を復唱します。「是」の場合だけ
実行し、「不是」の場合は商品名の聞き取りからやり直します。不明瞭な回答の場合は安全のため
操作をキャンセルし、自動再実行によるループを防ぎます。
