...
test('read-only audit handles mixed kinds and rejects mixed occurrence states',async()=>{
 ...
 const calls=[];const adapter={async locateTargets(rows){calls.push(rows);return rows.map(r=>({...r,matches:[{rowMatches:r.kind==='item'?[{hidden:false,unavailable:true}]:[{unavailable:false},{unavailable:true}]}]}));}};
