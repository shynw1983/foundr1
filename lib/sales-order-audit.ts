...
export type SalesOrderAuditReport = {
  checkedAt: string;
  summary: CountRow;
  bySource: CountRow[];
  itemSummary: CountRow;
  orphanSalesOrders: CountRow[];
  externalOnlySalesOrders: CountRow[];
  dependencyCounts: CountRow[];
  hasProblems: boolean;
};

export async function auditSalesOrders(): Promise<SalesOrderAuditReport> {
  ...
  ].some((value) => toNumber(value) > 0) || orphanSalesOrders.length > 0;
...
export function summarizeSalesOrderAuditProblem(report: SalesOrderAuditReport) {
  const orphanCount = report.orphanSalesOrders.reduce((sum, row) => sum + toNumber(row.orders), 0);
  return [
    `漏同期 ${toNumber(report.summary.missing_sales_orders)}件`,
    `金額差異 ${toNumber(report.summary.amount_mismatches)}件`,
    `状態差異 ${toNumber(report.summary.status_mismatches)}件`,
    `支払状態差異 ${toNumber(report.summary.payment_status_mismatches)}件`,
    `明細差異 ${toNumber(report.itemSummary.item_count_mismatches) + toNumber(report.itemSummary.item_total_mismatches)}件`,
    `残存 sales_orders ${orphanCount}件`
  ].join(" / ");
}

export async function notifySalesOrderAuditProblems(report: SalesOrderAuditReport) {
  if (!report.hasProblems) return { notified: 0 };

  const recentRows = await sql`
    select count(*)::int as count
    from os_notifications
    where notification_type = 'sales_order_audit_problem'
      and created_at > now() - interval '12 hours'
  `;
  if (Number(recentRows[0]?.count ?? 0) > 0) return { notified: 0, skipped: "recent_notification_exists" };
  ...
  `;
  const message = summarizeSalesOrderAuditProblem(report);
  for (const recipient of recipients) {
    await createOsNotification({
      employeeId: String(recipient.id),
      type: "sales_order_audit_problem",
      title: "sales_orders 同期チェックで確認が必要です",
      message,
      href: "/os/sales",
      sendPush: false
    });
