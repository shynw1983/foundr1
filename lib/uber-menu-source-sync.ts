...
export async function ingestUberMenuSource(input: { sourceId: string; commandId: string; storeId: string; catalog: unknown; dryRun?: boolean; verifyRollback?: boolean; bootstrap?: boolean }) {
  ...
  for (const node of nodes) {
    ...
    if (node.kind==='item') {
      ...
      statements.push(sql`insert into menu_catalog_items(id,brand_id,external_id,name,display_names,description,image_url,base_price,category,variable_schema,sort_order) values(${node.targetId},${source.brand_id},${node.uberId},${node.name},${translations}::jsonb,${node.description},${node.imageUrl},${node.price},${categoryName},${variables}::jsonb,${node.sortOrder}) on conflict(id) do update set promotion_prefix='',name=excluded.name,display_names=menu_catalog_items.display_names||excluded.display_names,description=excluded.description,image_url=excluded.image_url,base_price=excluded.base_price,category=excluded.category,variable_schema=excluded.variable_schema,sort_order=excluded.sort_order,is_active=true,updated_at=now()`);
