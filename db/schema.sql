...
alter table menu_external_platforms add column if not exists rule_version text not null default '';
alter table menu_external_platforms add column if not exists rule_config jsonb not null default '{}'::jsonb;

create table if not exists menu_platform_target_settings (
  id uuid primary key default gen_random_uuid(),
  brand_id uuid not null references brands(id) on delete cascade,
  store_id uuid references stores(id) on delete cascade,
  ...
  price_override numeric(12, 2),
  emoji_mode text not null default 'follow',
  placement_config jsonb not null default '{}'::jsonb,
  updated_by uuid references employees(id) on delete set null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (external_platform_id, target_type, target_id),
  check (target_type in ('item', 'option', 'category', 'option_group')),
  check (emoji_mode in ('follow', 'show', 'hide'))
);

create table if not exists menu_platform_object_mappings (
  id uuid primary key default gen_random_uuid(),
  brand_id uuid not null references brands(id) on delete cascade,
  store_id uuid references stores(id) on delete cascade,
  external_platform_id uuid not null references menu_external_platforms(id) on delete cascade,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (external_platform_id, target_type, external_id),
  check (target_type in ('item', 'option', 'category', 'option_group'))
);

-- A logical product can occur several times on a merchant menu. Keep every
-- verified physical ID; never let the last observation replace the others.
...
create table if not exists menu_platform_snapshots (
  id uuid primary key default gen_random_uuid(),
  brand_id uuid not null references brands(id) on delete cascade,
  store_id uuid references stores(id) on delete cascade,
  external_platform_id uuid not null references menu_external_platforms(id) on delete cascade,
...
create table if not exists menu_platform_import_candidates (
  id uuid primary key default gen_random_uuid(),
  brand_id uuid not null references brands(id) on delete cascade,
  store_id uuid references stores(id) on delete cascade,
  external_platform_id uuid not null references menu_external_platforms(id) on delete cascade,
  ...
  updated_at timestamptz not null default now(),
  unique (external_platform_id, target_type, external_id),
  check (target_type in ('item', 'option')),
  check (status in ('pending', 'ignored', 'adopted', 'not_seen'))
);

create table if not exists menu_publish_batches (
  id uuid primary key default gen_random_uuid(),
  brand_id uuid not null references brands(id) on delete cascade,
  store_id uuid references stores(id) on delete cascade,
  status text not null default 'draft',
...
alter table menu_change_sync_tasks add column if not exists publish_batch_id uuid references menu_publish_batches(id) on delete cascade;
alter table menu_change_sync_tasks add column if not exists rule_version text not null default '';
alter table menu_change_sync_tasks add column if not exists current_value jsonb not null default '{}'::jsonb;
alter table menu_change_sync_tasks add column if not exists projected_value jsonb not null default '{}'::jsonb;
alter table menu_change_sync_tasks add column if not exists command_id uuid;
alter table menu_change_sync_tasks add column if not exists phase text not null default 'queued';
alter table menu_change_sync_tasks add column if not exists attempts integer not null default 0;
alter table menu_change_sync_tasks add column if not exists max_attempts integer not null default 3;
alter table menu_change_sync_tasks add column if not exists error_code text not null default '';
alter table menu_change_sync_tasks add column if not exists error_detail text not null default '';
alter table menu_change_sync_tasks add column if not exists is_retryable boolean not null default true;
alter table menu_change_sync_tasks add column if not exists verified_at timestamptz;
...
create table if not exists store_dining_sessions (
  ...
  completed_at timestamptz,
  updated_at timestamptz not null default now(),
  constraint store_dining_sessions_status_check
...
create index if not exists store_dining_session_orders_session_idx
  on store_dining_session_orders(session_id, linked_at);
...
select
  stores.id,
  'maamaa-sakuranamiki-v1',
  '/store/maamaa-floor-background.svg',
  800,
  1200,
...
create index if not exists idx_menu_publish_batches_brand_created on menu_publish_batches(brand_id, created_at desc);
create index if not exists idx_menu_platform_target_settings_target on menu_platform_target_settings(brand_id, target_type, target_id);
create index if not exists idx_menu_platform_object_mappings_target on menu_platform_object_mappings(brand_id, target_type, target_id);
create index if not exists idx_menu_platform_snapshots_latest on menu_platform_snapshots(external_platform_id, snapshot_type, captured_at desc);
create index if not exists idx_menu_platform_import_candidates_status on menu_platform_import_candidates(brand_id, status, last_seen_at desc);
create index if not exists idx_menu_change_sync_tasks_batch on menu_change_sync_tasks(publish_batch_id, status, created_at);
